...
(() => {
    ...
    while (queue.size > 0) {
      ...
      for (const [neighbor, distanceType] of vertex.neighbors.entries()) {
        ...
        switch (distanceType) {
          ...
          case "guaranteed11": {
            distance.addCat(neighbor.catName);
            let start: TrackGraphNode | null = vertex;
            for (let i = 0; i < 10; i++) {
              if (!start) break;
  ...
  setTimeout(() => {
    // copy available cats
    const selector =
        document.querySelector<HTMLSelectElement>("#bstu-selector")!,
      ...
      catFoodInput =
        document.querySelector<HTMLInputElement>("#bstu-cat-food")!,
      providedCatSelector =
    ...
    function loadFromLocalStore() {
      ...
      for (const cat of data.cats ?? []) {
        if (
          ![...selector.options].find((option) => option.textContent === cat)
        ) {
          continue;
        }
        addChoice(cat);
